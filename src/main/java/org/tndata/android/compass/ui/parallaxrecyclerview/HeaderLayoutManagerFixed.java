package org.tndata.android.compass.ui.parallaxrecyclerview;
...
public class HeaderLayoutManagerFixed extends RecyclerView.LayoutManager {
     ...
     * this interface helps {@link HeaderLayoutManagerFixed} make those
     * decisions. Based on {@link #mOrientation}, an implementation is lazily
     * created in {@link #ensureRenderState} method.
     */
     ...
     */
    public HeaderLayoutManagerFixed(Context context, int orientation,
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            if (didLayoutFromEnd) {
                ...
                state.mAnchorPosition = getPosition(refChild);
                state.mAnchorOffset = mOrientationHelper
                        .getDecoratedStart(refChild)
    ...
    public PointF computeScrollVectorForPosition(int targetPosition) {
        ...
        final int direction = targetPosition < firstChildPos != mShouldReverseLayout ? -1
                : 1;
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler,
        ...
        if (mPendingScrollPosition != RecyclerView.NO_POSITION) {
            ...
            if (mPendingSavedState != null) {
                ...
                if (layoutFromEnd) {
                    ...
                    anchorCoordinate = mOrientationHelper
                ...
                if (child != null) {
                    ...
                    if (childSize > mOrientationHelper.getTotalSpace()) {
                        ...
                        anchorCoordinate = layoutFromEnd ? mOrientationHelper
                                .getDecoratedEnd(child) : mOrientationHelper
                                .getDecoratedStart(child);
                    }
            ...
            if (layoutFromEnd) {
                ...
                anchorCoordinate = mOrientationHelper
                        .getDecoratedStart(referenceChild);
                anchorItemPosition = getPosition(referenceChild);
            }
        } else {
            anchorCoordinate = layoutFromEnd ? mOrientationHelper
        ...
        if (getChildCount() > 0 && !state.isPreLayout()
            ...
            final List<RecyclerView.ViewHolder> scrapList = recycler
                    .getScrapList();
            ...
            for (int i = 0; i < scrapSize; i++) {
                ...
                if (direction == RenderState.LAYOUT_START) {
                    scrapExtraStart += mOrientationHelper
                            .getDecoratedMeasurement(scrap.itemView);
                } else {
                    scrapExtraEnd += mOrientationHelper
                            .getDecoratedMeasurement(scrap.itemView);
                }
            }

            if (DEBUG) {
                Log.d(TAG, "for unused scrap, decided to add "
                        + scrapExtraStart + " towards start and "
    ...
    private void updateRenderStateToFillStart(int itemPosition, int offset) {
        mRenderState.mAvailable = offset
                - mOrientationHelper.getStartAfterPadding();
        mRenderState.mCurrentPosition = itemPosition;
    ...
    public int computeHorizontalScrollOffset(RecyclerView.State state) {
        ...
        return mShouldReverseLayout ? state.getItemCount() - 1 - topPosition
                : topPosition;
    }
    ...
    private void updateRenderState(int layoutDirection, int requiredSpace,
        ...
        if (layoutDirection == RenderState.LAYOUT_END) {
            ...
            mRenderState.mItemDirection = mShouldReverseLayout ? RenderState.ITEM_DIRECTION_HEAD
                    : RenderState.ITEM_DIRECTION_TAIL;
            mRenderState.mCurrentPosition = getPosition(child)
    ...
    private int scrollBy(int dy, RecyclerView.Recycler recycler,
        ...
        ensureRenderState();
        final int layoutDirection = dy > 0 ? RenderState.LAYOUT_END
    ...
    private void recycleChildren(RecyclerView.Recycler recycler,
        ...
        if (DEBUG) {
            Log.d(TAG, "Recycling " + Math.abs(startIndex - endIndex)
                    + " items");
        }
     ...
     *      int)
     */
    private void recycleByRenderState(RecyclerView.Recycler recycler,
    ...
    private int fill(RecyclerView.Recycler recycler, RenderState renderState,
        ...
        while (remainingSpace > 0 && renderState.hasMore(state)) {
            ...
            layoutDecorated(view, left + params.leftMargin, top
                    + params.topMargin, right - params.rightMargin, bottom
                    - params.bottomMargin);
            if (DEBUG) {
    ...
    int findOneVisibleChild(int fromIndex, int toIndex,
    ...
    private static class RenderState {
        ...
        private View nextFromLimitedList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final int distance = (viewHolder.getPosition() - mCurrentPosition)
                        * mItemDirection;
                if (distance < 0) {
    ...
    public void setHeaderIncrementFixer(final View headerIncrementFixer) {
        headerIncrementFixer.getViewTreeObserver().addOnGlobalLayoutListener(
                new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                            headerIncrementFixer.getViewTreeObserver()
                                    .removeOnGlobalLayoutListener(this);
                        } else {
                            headerIncrementFixer.getViewTreeObserver()
                                    .removeGlobalOnLayoutListener(this);
                        }
                        mHeaderIncrementFixer = headerIncrementFixer
                                .getHeight();
                    }
                });
