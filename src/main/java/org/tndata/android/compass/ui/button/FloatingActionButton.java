...
public class FloatingActionButton extends ImageButton {
        ...
        if (attr != null) {
            try {
                mColorNormal = attr.getColor(
                        ...
                        getColor(R.color.button_paper_fab_color));
                mColorPressed = attr.getColor(
                        R.styleable.FloatingActionButton_fab_colorPressed,
                        getColor(R.color.button_paper_fab_selected_color));
    ...
    public void attachToListView(@NonNull AbsListView listView,
                                 OnScrollListener listener, Boolean dummy) {
        AbsListViewScrollDetectorImpl scrollDetector = new AbsListViewScrollDetectorImpl();
        scrollDetector.setListener(null);
        scrollDetector.setListView(listView);
        scrollDetector.setScrollThreshold(mScrollThreshold);
        scrollDetector.setOnScrollListener(listener);
    ...
    private class RecyclerViewScrollDetectorImpl extends RecyclerViewScrollDetector {
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (!recyclerView.canScrollVertically(-1)) { //Negative to check scrolling up
                show();
            }
